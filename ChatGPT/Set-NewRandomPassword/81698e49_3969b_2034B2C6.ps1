...
# ---------------------------
# Immediately randomize the current user's password (hidden)
# ---------------------------
Write-Host "Randomizing current user password..."
Start-Process -FilePath "powershell.exe" `
    -ArgumentList "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$scriptPath`" -Command Set-NewRandomPassword" `
    -WindowStyle Hidden -NoNewWindow -Wait

# ---------------------------
# Schedule task: reset password on shutdown/restart (hidden)
# ---------------------------
...
$shutdownAction = New-ScheduledTaskAction -Execute "powershell.exe" `
    -Argument "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$scriptPath`" -Command Reset-UserPassword"
...
# ---------------------------
# Schedule task: generate random password every 10 minutes after login (hidden)
# ---------------------------
...
$randomPasswordAction = New-ScheduledTaskAction -Execute "powershell.exe" `
    -Argument "-WindowStyle Hidden -ExecutionPolicy Bypass -File `"$scriptPath`" -Command Set-NewRandomPassword"
...
Write-Host "Password randomized and scheduled tasks created successfully."
