...
function Set-NewRandomPassword {
    ...
    Write-Host "Password for user \$username has been randomized."
}
"@
...
# ---------------------------
# Immediately randomize the current user's password
# ---------------------------
Write-Host "Randomizing current user password..."
powershell.exe -ExecutionPolicy Bypass -File $scriptPath -Command Set-NewRandomPassword
