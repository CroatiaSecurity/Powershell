...
# Registers a persistent scheduled task that runs at every logon

$ErrorActionPreference = 'SilentlyContinue'

# Apply immediately for current user
Set-ItemProperty -Path 'Registry::HKCU\Control Panel\NotifyIconSettings\*' -Name 'IsPromoted' -Value 1 -Type 'DWord'

# Register scheduled task for persistence across logons
$taskXml = @'
<Task version="1.2" xmlns="http://microsoft.com">
  <Triggers>
    <LogonTrigger>
      <Repetition>
