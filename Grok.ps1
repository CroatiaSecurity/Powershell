...
# ============================================================================
# Grok.ps1 - Unified single-file EDR/Antivirus (AV base + Detection monitors)
# Author: Gorstak
# Version: 3.0.0 (Grok.ps1 unified)
#
# Changelog:
...
#   v3.0.0 - Unified Grok.ps1: AV base + Detection monitors (Cookie, CursorTakeover, FakeUAC, LNK, Scareware)
#            Replaces 3.2MB Grok export library dump. Run: .\Grok.ps1 | -Uninstall | -LearningMode | -SelfTest
#
...
# USAGE:
#   Normal run:    .\Grok.ps1
#   Uninstall:     .\Grok.ps1 -Uninstall   (removes persistence, stops instances)
#   Test/chaos:    .\Grok.ps1 -ChaosMode   (validates detections with EICAR)
#   Learning:      .\Grok.ps1 -LearningMode (log-only, no kill/quarantine)
#   Self-test:     .\Grok.ps1 -SelfTest    (run diagnostics and exit)
...
$Script:ManagedJobConfig = @{
    ...
    CookieIntegrityMonitorIntervalSeconds = 300
    CursorTakeoverDetectionIntervalSeconds = 3
    FakeUacWindowDetectionIntervalSeconds = 10
    LnkUncProtectionIntervalSeconds = 300
    RansomwareScarewareDetectionIntervalSeconds = 10
}
...
$Config = @{
    ...
    EnableCookieIntegrityMonitor = $true      # Chrome cookie DB hash integrity (session hijack)
    EnableCursorTakeoverDetection = $true     # Low-variance automated cursor movement
    EnableFakeUacWindowDetection = $true      # Non-system process with UAC/security dialog titles
    EnableLnkUncProtection = $true            # Remove Desktop/StartMenu/Taskbar LNKs pointing to UNC
    EnableRansomwareScarewareDetection = $true # Window-title ransomware/scareware keywords
}
...
# ===================== Detection/ folder monitors (v3.0.0 merge) =====================

# --- CookieIntegrityMonitor (from Detection/CookieMonitor.ps1) ---
# Alert-only on cookie DB hash change (does not kill Chrome or force-restore).
function Invoke-CookieIntegrityMonitor {
    if (-not $Config.EnableCookieIntegrityMonitor) { return 0 }
    $detections = 0
    try {
        $cookiePaths = @(
        ...
        $stateDir = Join-Path $Config.DatabasePath "cookie_integrity"
        if (-not (Test-Path $stateDir)) { New-Item -ItemType Directory -Path $stateDir -Force -ErrorAction SilentlyContinue | Out-Null }
        if (-not $Script:CookieIntegrityState) { $Script:CookieIntegrityState = @{} }

        foreach ($cpath in $cookiePaths) {
            if (-not (Test-Path $cpath)) { continue }
            try {
                $hash = (Get-FileHash -Path $cpath -Algorithm SHA256 -ErrorAction Stop).Hash
                $key = $cpath.ToLowerInvariant()
                $stateFile = Join-Path $stateDir (($key -replace '[\\/:*?"<>|]', '_') + ".hash")
                $prev = $null
                if ($Script:CookieIntegrityState.ContainsKey($key)) { $prev = $Script:CookieIntegrityState[$key] }
                elseif (Test-Path $stateFile) { $prev = (Get-Content $stateFile -Raw -ErrorAction SilentlyContinue).Trim() }

                if ($prev -and $prev -ne $hash) {
                    $detections++
                    Write-AVLog "Cookie integrity change: $cpath (session hijack / cookie theft possible)" "THREAT"
                    try {
                        if (Get-Command Invoke-ResponseAction -ErrorAction SilentlyContinue) {
                            Invoke-ResponseAction -ThreatType "CookieIntegrity" -ProcessName "browser" -ProcessId 0 -Evidence "Cookie DB hash changed: $cpath" -Severity "Medium"
                        }
                    } catch {}
                }
                $Script:CookieIntegrityState[$key] = $hash
                Set-Content -Path $stateFile -Value $hash -Force -ErrorAction SilentlyContinue
            } catch {}
        }
    } catch {
        Write-AVLog "CookieIntegrityMonitor error: $_" "WARN"
    }
    return $detections
}

# --- CursorTakeoverDetection (from Detection/CursorTakeoverDetection.ps1) ---
function Invoke-CursorTakeoverDetection {
    if (-not $Config.EnableCursorTakeoverDetection) { return 0 }
    $detections = 0
    try {
        if (-not ([System.Management.Automation.PSTypeName]'CursorProbeAV').Type) {
            Add-Type @"
using System;
using System.Runtime.InteropServices;
public static class CursorProbeAV {
...
"@ -ErrorAction SilentlyContinue
        ...
        $pt = New-Object CursorProbeAV+POINT
        if ([CursorProbeAV]::GetCursorPos([ref]$pt)) {
            $now = Get-Date
            ...
            if ($script:CursorSamples.Count -ge 12) {
                ...
                $var = 0.0
                foreach ($d in $deltas) { $var += [Math]::Pow(($d - $mean), 2) }
                $var = $var / [Math]::Max($deltas.Count, 1)
                # Low variance + continuous movement => bot/takeover style cursor
                if ($var -lt 0.005 -and $mean -gt 0.01) {
                    $key = "cursor_takeover"
                    $last = if ($script:CursorTakeoverLastAlert) { $script:CursorTakeoverLastAlert } else { [datetime]::MinValue }
                    if (((Get-Date) - $last).TotalSeconds -gt 60) {
                        $detections++
                        $script:CursorTakeoverLastAlert = Get-Date
                        Write-AVLog "Possible cursor takeover / automated movement (var=$([Math]::Round($var,6)) mean=$([Math]::Round($mean,4)))" "THREAT"
                    }
                }
            }
        }
    } catch {
        Write-AVLog "CursorTakeoverDetection error: $_" "WARN"
    }
    return $detections
}

# --- FakeUacWindowDetection (from Detection/FakeUacDetection.ps1) ---
function Invoke-FakeUacWindowDetection {
    if (-not $Config.EnableFakeUacWindowDetection) { return 0 }
    $detections = 0
    try {
        $susPatterns = @("user account control", "do you want to allow", "windows security", "microsoft defender", "critical update", "windows update")
        $trusted = @("consent", "explorer", "dwm", "applicationframehost", "securityhealthservice", "msmpeng", "systemsettings", "credentialuibroker", "logonui", "lockapp", "powershell", "pwsh")
        if (-not $script:FakeUacAlerted) { $script:FakeUacAlerted = @{} }
        foreach ($p in Get-Process -ErrorAction SilentlyContinue) {
            try {
                ...
                $hit = $false
                foreach ($pat in $susPatterns) { if ($title -like "*$pat*") { $hit = $true; break } }
                if (-not $hit) { continue }
                $pidKey = [string]$p.Id
                if ($script:FakeUacAlerted.ContainsKey($pidKey)) { continue }
                $script:FakeUacAlerted[$pidKey] = Get-Date
                $detections++
                Write-AVLog "Possible fake UAC/system dialog: $($p.ProcessName) (PID $($p.Id)) | $($p.MainWindowTitle)" "THREAT"
                if ($Config.AutoKillThreats -and -not $Script:LearningMode) {
        ...
        $keys = @($script:FakeUacAlerted.Keys)
        foreach ($k in $keys) {
            if ($script:FakeUacAlerted[$k] -lt $cutoff) { $script:FakeUacAlerted.Remove($k) }
        }
    } catch {
        Write-AVLog "FakeUacWindowDetection error: $_" "WARN"
    }
    return $detections
}

# --- LnkUncProtection (from Detection/LNKProtection.ps1) ---
function Invoke-LnkUncProtection {
    if (-not $Config.EnableLnkUncProtection) { return 0 }
    $detections = 0
    try {
        ...
        foreach ($path in $paths) {
            ...
            Get-ChildItem $path -Recurse -Filter *.lnk -ErrorAction SilentlyContinue | ForEach-Object {
                try {
                    ...
                    $args = $sc.Arguments
                    $unc = ($target -like "\\*") -or ($args -like "*\\*")
                    # Also catch mshta/cmd launching remote payloads via args
                    $suspicious = $unc -or ($args -match '(?i)(powershell|cmd|mshta|wscript|cscript).{0,80}\\\\')
                    if ($suspicious) {
                        $detections++
                        Write-AVLog "Malicious LNK (UNC/remote): $($_.FullName) -> Target='$target' Args='$args'" "THREAT"
                        if ($Config.AutoQuarantine -and -not $Script:LearningMode) {
                            try {
                                if (Get-Command Invoke-QuarantineFile -ErrorAction SilentlyContinue) {
                                    Invoke-QuarantineFile -Path $_.FullName -Reason "LNK UNC/remote payload"
                                } else {
        ...
        Write-AVLog "LnkUncProtection error: $_" "WARN"
    }
    return $detections
}

# --- RansomwareScarewareDetection (from Detection/RansomwareScarewareDetection.ps1) ---
function Invoke-RansomwareScarewareDetection {
    if (-not $Config.EnableRansomwareScarewareDetection) { return 0 }
    $detections = 0
    try {
        $patterns = @("encrypted", "bitcoin", "decrypt", "ransom", "pay to unlock", "your files have been", "restore your files", "microsoft support", "pay fine", "your computer has been locked")
        $allow = @("explorer", "logonui", "lockapp", "consent", "applicationframehost", "steam", "epicgameslauncher", "chrome", "msedge", "firefox", "code", "devenv", "notepad")
        if (-not $script:ScarewareAlerted) { $script:ScarewareAlerted = @{} }
        foreach ($p in Get-Process -ErrorAction SilentlyContinue) {
            try {
                if (-not $p.MainWindowTitle) { continue }
                ...
                $pidKey = [string]$p.Id
                if ($script:ScarewareAlerted.ContainsKey($pidKey)) { continue }
                $script:ScarewareAlerted[$pidKey] = Get-Date
                $detections++
                Write-AVLog "Ransomware/scareware window: $($p.ProcessName) (PID $($p.Id)) | $($p.MainWindowTitle)" "THREAT"
                if ($Config.AutoKillThreats -and -not $Script:LearningMode) {
                    try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
                }
            } catch {}
        ...
        $cutoff = (Get-Date).AddMinutes(-10)
        foreach ($k in @($script:ScarewareAlerted.Keys)) {
            if ($script:ScarewareAlerted[$k] -lt $cutoff) { $script:ScarewareAlerted.Remove($k) }
        }
    } catch {
        Write-AVLog "RansomwareScarewareDetection error: $_" "WARN"
    }
    return $detections
}
...
try {
    ...
    Write-Host "`nGrok.ps1 Unified Protection (Single File)`n" -ForegroundColor Cyan
...
Write-Host "[PROTECTION] Anti-termination safeguards active" -ForegroundColor Green
    ...
    $moduleNames = @(
        ...
        "ShadowProxyCaptureDetection",
        "DragonBreathHunter"
        "CookieIntegrityMonitor",
        "CursorTakeoverDetection",
        "FakeUacWindowDetection",
        "LnkUncProtection",
        "RansomwareScarewareDetection"
    )
    ...
    Write-Host "  Grok.ps1 Protection ACTIVE" -ForegroundColor Green
