...
function Convert-ToRegFormat {
    ...
    # Match reg add commands with flexible quoting
...
# Function to handle service disabling
function Convert-ServiceToRegFormat {
    param (
        [string]$ServiceName,
        [string]$SectionComment
    )
    return "; $SectionComment`n[HKEY_LOCAL_MACHINE\\SYSTEM\\CurrentControlSet\\Services\\$ServiceName]`n`"Start`"=dword:00000004`n"
}

# Function to handle NTP settings
...
foreach ($line in $batchContent -split "`n") {
    ...
    if ($line -match 'Set-ItemProperty.*HKLM:\\SYSTEM\\CurrentControlSet\\Services\\([^\\]+).*Start.*4') {
        ...
        $regEntry = Convert-ServiceToRegFormat -ServiceName $serviceName -SectionComment $currentSection
        $regTweaks += $regEntry
    }
