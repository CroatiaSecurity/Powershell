...
function Parse-Rules {
    ...
    foreach ($indicator in $indicators) {
        ...
        if (-not $seenIndicators.ContainsKey($key)) {
            ...
            $existingSources = if ($existing.Source) { $existing.Source -split ',' | ForEach-Object { $_.Trim() } } else { @() }
            $existingRuleFiles = if ($existing.RuleFile) { $existing.RuleFile -split ',' | ForEach-Object { $_.Trim() } } else { @() }
