...
function Monitor-Processes {
    ...
    try {
        ...
        foreach ($event in $events) {
            ...
            $processName = ($eventData | Where-Object { $_.Name -eq "NewProcessName" } | Select-Object -First 1).'#text' ?? "Unknown"
            $processId = ($eventData | Where-Object { $_.Name -eq "NewProcessId" } | Select-Object -First 1).'#text' ?? "Unknown"
            $parentProcessName = ($eventData | Where-Object { $_.Name -eq "ParentProcessName" } | Select-Object -First 1).'#text' ?? "Unknown"
