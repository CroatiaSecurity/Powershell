...
function Parse-Rules {
    ...
    $uniqueIPs = ($ipList | Sort-Object -Property Value -Unique | Measure-Object).Count
    $uniqueDomains = ($domainList | Sort-Object -Property Value -Unique | Measure-Object).Count
    ...
    Write-Log "Deduplicating $($indicators.Count) indicators into unique set..."
    foreach ($group in $indicatorGroups) {
        $firstIndicator = $group.Group[0]
        ...
        $uniqueIndicator = @{
            Type = $firstIndicator.Type
            Value = $firstIndicator.Value
            Source = if ($sources -and $sources -ne "Unknown") { $sources } else { "Unknown" }
            RuleFile = if ($ruleFiles -and $ruleFiles -ne "Unknown") { $ruleFiles } else { "Unknown" }
        }
        $uniqueIndicators += $uniqueIndicator
        Write-Log "Deduplicated indicator: Type=$($uniqueIndicator.Type), Value=$($uniqueIndicator.Value), Source=$($uniqueIndicator.Source), RuleFile=$($uniqueIndicator.RuleFile)"
    }
