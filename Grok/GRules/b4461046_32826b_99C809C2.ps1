...
function Ensure-ProcessAuditing {
    try {
        ...
        if ($auditStatus.'Success Auditing' -ne "Enable" -or $auditStatus.'Failure Auditing' -ne "Enable") {
            ...
            $result = Start-Process -FilePath "auditpol.exe" -ArgumentList "/set /subcategory:`"Process Creation`" /success:enable /failure:enable" -NoNewWindow -Wait -PassThru
            if ($result.ExitCode -ne 0) {
                Write-Log "Failed to enable process creation auditing. auditpol exit code: $($result.ExitCode)" -EntryType "Error"
                $Global:ExitCode = 1
                return $false
            }
            # Verify again
            $auditStatus = auditpol /get /subcategory:"Process Creation" /r | ConvertFrom-Csv
            if ($auditStatus.'Success Auditing' -ne "Enable" -or $auditStatus.'Failure Auditing' -ne "Enable") {
                Write-Log "Failed to enable process creation auditing after execution" -EntryType "Error"
                $Global:ExitCode = 1
                return $false
            }
...
function Monitor-Processes {
    ...
    try {
        $events = Get-WinEvent -LogName "Security" -FilterXPath "*[System[(EventID=1)]]" -MaxEvents $Global:Config.Telemetry.MaxEvents
...
try {
    ...
    if (-not (Ensure-ProcessAuditing)) {
        Write-Log "Process creation auditing is required for process monitoring. Exiting." -EntryType "Error"
        $Global:ExitCode = 1
