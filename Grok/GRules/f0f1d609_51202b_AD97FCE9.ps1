...
function Parse-Rules {
    ...
    $indicators | Group-Object -Property Type, Value | ForEach-Object {
        $group = $_.Group
        $uniqueIndicator = $group[0]
        $sources = ($group | Select-Object -ExpandProperty Source -Unique) -join ','
        $ruleFiles = ($group | Select-Object -ExpandProperty RuleFile -Unique) -join ','
        $uniqueIndicator.Source = if ($sources) { $sources } else { "Unknown" }
        $uniqueIndicator.RuleFile = if ($ruleFiles) { $ruleFiles } else { "Unknown" }
...
function Monitor-Processes {
    ...
    try {
        ...
        if ($events) {
            ...
            Write-Log "No process events found matching the criteria" -EntryType "Information"
        }
    }
    catch {
