...
public class KeyScrambler {
    ...
    public static void Start(IntPtr consoleWnd) {
        ...
        Console.WriteLine("Key scrambler ACTIVE (silent injection mode)");
        MSG msg;
    ...
    public static void Stop() {
        if (_hookID != IntPtr.Zero) {
            ...
            _hookID = IntPtr.Zero;
            Console.WriteLine("Key scrambler stopped.");
        }
    }

    private static IntPtr HookCallback(int nCode, IntPtr wParam, IntPtr lParam) {
        if (nCode >= 0 && wParam == (IntPtr)WM_KEYDOWN) {
            // Skip if GSecurity console is foreground
            if (GetForegroundWindow() == _consoleWnd) {
                return CallNextHookEx(_hookID, nCode, wParam, lParam);
            }

            // Inject ONE random A-Z as pure Unicode (no duplicates)
            ushort fakeChar = (ushort)_rnd.Next(65, 91); // A-Z
...
function Start-KeyScrambler {
    $consoleWnd = (Get-Process -Id $PID).MainWindowHandle
    $job = Start-Job -ScriptBlock {
        param($wnd)
        try {
            [KeyScrambler]::Start($wnd)
        } catch {
            Write-Host "Scrambler error: $_"
        }
    } -ArgumentList $consoleWnd
    Write-Log "Key scrambler started in background (Job ID: $($job.Id))" -Level Info
    return $job
}
...
function Stop-KeyScrambler {
    ...
    if ($Job) {
        ...
        Write-Log "Key scrambler stopped and job cleaned up." -Level Info
    }
}
...
function Main {
    ...
    # === START KEY SCRAMBLER ===
    $Global:ScramblerJob = Start-KeyScrambler
