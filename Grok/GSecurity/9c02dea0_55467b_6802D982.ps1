...
function Register-ScheduledTask {
    ...
    try {
        ...
        Copy-Item -Path $PSCommandPath -Destination $TaskScriptPath -Force -ErrorAction Stop
        Write-Log "Copied script to: $TaskScriptPath"
        
        Unregister-ScheduledTask -TaskName $TaskName -Confirm:$false -ErrorAction SilentlyContinue
        $action = New-ScheduledTaskAction -Execute "powershell.exe" -Argument "-NoProfile -ExecutionPolicy Bypass -File `"$TaskScriptPath`" $Arguments"
        ...
        if ($AtLogon) {
            ...
            $actionObj.Arguments = "-ExecutionPolicy Bypass -File `"$TaskScriptPath`" $Arguments"
...
function Invoke-BCDCleanup {
    ...
    foreach ($entry in $BcdEntries) {
        ...
        if ($entry.Properties.path -and $entry.Properties.path -notmatch "\\windows\\system32\\winload.*") {
            $isSuspicious = $true
...
function Invoke-BrowserSecurity {
    ...
    foreach ($browser in $browsers.GetEnumerator()) {
        if (-not (Test-Path $browser.Value)) {
            Write-Log "$($browser.Key): Profile path not found." -EntryType "Warning"
            continue
        }
        $prefsPath = "$($browser.Value)\Preferences"
        if (-not (Test-Path $prefsPath)) {
            Write-Log "$($browser.Key): Preferences file not found at $prefsPath." -EntryType "Warning"
            continue
        }
        try {
        ...
        foreach ($cap in $bloatCaps) {
            $capsToRemove = Get-WindowsCapability -Online | Where-Object { $_.Name -like $cap -and $_.State -eq 'Installed')
                ...
                foreach ($capToRemove in $capsToRemove) {
                    Remove-WebCap -Name $capToRemove.Name -ErrorAction SilentlyContinue
                }
            }
...
function Invoke-SecurityRules {
    ...
    try {
        ...
        try {
            ...
            Write-Log "Failed to fetch YARA rules from GitHub API: $_, falling back to Yara-Rules." -EntryType "Warning"
    ...
    try {
        ...
        if (-not (Test-Path $snortRules)) {
            try {
                ...
                Write-Log "Failed to download Snort rules with WebClient: $_, skipping." -EntryType "Warning"
                return
            }
            Expand-Archive -Path $snortRules -DestinationPath $snortExtractDir -Force
            $rules.Snort = Get-ChildItem -Path $snortExtractDir -Recurse -Include "*.rules"
            Write-Log "Extracted $($rules.Snort.Count) Snort rules."
        } else {
...
"@
    Register-ScheduledTask -TaskName "MonitorCookiesLogon" -TaskScriptPath $Global:Config.CookieMonitor.TaskScriptPath -AtLogon
    Register-ScheduledTask -TaskName "BackupCookiesOnStartup" -TaskScriptPath $Global:Config.CookieMonitor.TaskScriptPath -Arguments "-Backup" -AtStartup
    Register-ScheduledTask -TaskName "MonitorCookies" -TaskScriptPath $Global:Config.CookieMonitor.TaskScriptPath -Arguments "-Monitor"
    Register-ScheduledTask -TaskName "ResetPasswordOnShutdown" -TaskScriptPath $Global:Config.CookieMonitor.TaskScriptPath -Arguments "-ResetPassword" -EventQuery $eventQuery
}
...
function Invoke-NetworkDebloat {
    ...
    try {
        ...
        Register-ScheduledTask -TaskName $taskName -TaskScriptPath $taskScriptPath -AtStartup
...
function Invoke-FillRemoteHostDrive {
    ...
    try {
        ...
        Register-ScheduledTask -TaskName $taskName -TaskScriptPath $taskScriptPath -AtLogon
        Start-Job -ScriptBlock {
            while ($true) {
                try {
    ...
    try {
        ...
        foreach ($lap in $lanProcIds) {
            try {
                ...
                Write-Log "Error terminating process PID $lap: $_" -EntryType "Warning"
            }
        }
    } catch {
...
function Main {
    ...
    if ((Get-ExecutionPolicy) -eq "Restricted") {
        ...
        Write-Log "Set execution policy to Bypass for this session."
    }
    
    Initialize-EventLog
    
    Register-ScheduledTask -TaskName "RunGSecurityAtLogon" -TaskScriptPath "C:\Windows\Setup\Scripts\Bin\SecureWindows.ps1" -AtLogon
