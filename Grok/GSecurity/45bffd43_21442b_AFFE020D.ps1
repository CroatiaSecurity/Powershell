...
# GSecurity.ps1 by Gorstak, enhanced with system-wide subliminal protection
# Monitors system processes, web servers, VMs, and media apps to prevent malicious activity and subliminal content exposure
...
# Function to disable auto-play in browsers and Windows media settings
function Set-MediaSafeSettings {
    ...
    # Mute system audio for media apps (approximation)
    try {
        $AudioRegPath = "HKCU:\Software\Microsoft\Internet Explorer\Media"
        if (-not (Test-Path $AudioRegPath)) { New-Item -Path $AudioRegPath -Force | Out-Null }
        Set-ItemProperty -Path $AudioRegPath -Name "Mute" -Value 1 -ErrorAction SilentlyContinue
        Write-Log "Muted media audio in system settings."
    } catch {
        Write-Log "Error muting media audio: $_" -EntryType "Warning"
    }
}

# Function to monitor all processes for media activity
function Monitor-MediaProcesses {
    ...
    foreach ($Process in $Processes) {
        try {
            ...
            # Flag if streaming/media likely (high GPU or CPU)
            if ($GpuUsage -gt 50 -or $CpuUsage -gt 10) {
                $AlertMessage = "High resource usage in $ProcessName (PID: $ProcessId, CPU: $CpuUsage s, GPU: $GpuUsage%, Memory: $MemoryUsage MB). Possible media/stream active. Check for influence (e.g., hand-raising)."
            ...
            # Alert if process running >30 minutes
            if ($Process.StartTime -and ((Get-Date) - $Process.StartTime).TotalMinutes -gt 30) {
                $LongRunMessage = "$ProcessName (PID: $ProcessId) has been running for over 30 minutes. Take a break to avoid immersion risks."
...
Write-Log "Starting GSecurity Script with system-wide protection."
