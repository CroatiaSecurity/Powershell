                    ...
                    try {
                        ...
                        if ($fileHash) {
                            if ($scannedFiles.ContainsKey($fileHash.Hash)) {
                                ...
                                $sc scantFiles[$fileHash.Hash] = $isValid
    ...
    try {
        ...
        foreach ($pid in $lanProcIds) {
            try {
                ...
                Write-GSecurityLog "Error processing PID $pid: $($_.ToString())" "Warning"
            }
        }
    } catch {
        Write-GSecurityLog "Error during rootkit detection: $($_.ToString())" "Error"
    }
}
...
function Register-SystemLogonScript {
    ...
    try {
        ...
        Write-GSecurityLog "Failed to register task: $_" "Error"
        exit 1
    }
}
