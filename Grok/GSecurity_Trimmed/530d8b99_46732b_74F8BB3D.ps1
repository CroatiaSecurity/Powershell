...
# GSecurity_Trimmed.ps1
# Trimmed Windows security script focusing on security rules
# Author: Gorstak, optimized by Grok
# Description: Contains only the security rules (YARA, Sigma, Snort) related functions
...
function Main {
    ...
    $indicators = Parse-Rules -Rules $rules -Config $Global:Config
    Apply-SecurityRules -Indicators $indicators -Config $Global:Config
    if ($Monitor) {
        Start-ProcessMonitor -Indicators $indicators -Config $Global:Config
    }
