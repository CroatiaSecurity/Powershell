...
# CosmicShield-Auto.ps1 - MMO-Friendly Automatic Background Protection for Windows 10/11
...
# ENHANCED: Whitelists .ps1/.bat in C:\Users, .ps1/.bat/.exe in Program Files, plus MMO exes and dirs.
...
# Static whitelist for system processes
...
Write-Log "=== Cosmic Shield Auto-Mode Started - System-Wide Protection (MMO-Friendly) ==="
...
while ($true) {
    ...
    if ($NetConns.Count -gt 0) {
        ...
        $NetConns | ForEach-Object { 
            ...
            # Skip if process is whitelisted (e.g., MMO client)
            if ($ProcName -and $Whitelist -contains $ProcName) {
                Write-Log "  - SKIPPED: Connection to $($_.RemoteAddress):$($_.RemotePort) by whitelisted $ProcName (Proc: $($_.OwningProcess))"
            } else {
    ...
    if ($RecentCount -gt 0) {
        ...
        if ($RecentCount -gt $MaxAutoQuarantine) {
            ...
            $ToQuarantine | ForEach-Object { 
                ...
                # Skip if file is in a game directory
