...
# CosmicShield-Auto.ps1 - Update-Friendly, DLL-Aware Background Protection for Windows 10/11
# Runs continuously, monitoring system-wide (all users) as SYSTEM.
# Auto-terminates suspicious processes, blocks net connections, quarantines files/DLLs.
# ENHANCED: Whitelists .ps1/.bat/.exe in C:\Users/Program Files/Games, updater processes, and update folders.
...
# Static whitelist for system processes, updaters, and common DLLs
...
$DLLWhitelist = @("kernel32.dll", "user32.dll", "gdi32.dll", "advapi32.dll", "ntdll.dll", "d3d11.dll", "dxgi.dll", "msvcrt.dll", "libcef.dll", "firefox.dll", "xul.dll")  # Added browser DLLs

# Directories to whitelist (games and update folders)
$GameDirs = @("C:\Program Files", "C:\Program Files (x86)", "C:\Games")  # Add your MMO paths
$UpdateDirs = @("C:\Program Files (x86)\Google\Update", "C:\Program Files (x86)\Mozilla Maintenance Service", "C:\Program Files (x86)\Steam", "C:\Users\*\AppData\Local\Google\Chrome\User Data", "C:\Users\*\AppData\Local\Mozilla\Updates", "C:\Users\*\AppData\Local\Temp")  # Common update paths

# Dynamic whitelist: Add .ps1, .bat from C:\Users; .ps1/.bat/.exe from Program Files, Games, Update dirs
...
Write-Log "=== Cosmic Shield Auto-Mode Started - System-Wide Protection (Update-Friendly) ==="
    ...
    if ($RecentCount -gt 0) {
        ...
        if ($RecentCount -gt $MaxAutoQuarantine) {
            ...
            $ToQuarantine | ForEach-Object { 
                ...
                if ($GameDirs + $UpdateDirs | Where-Object { $_.FullName -like "$_*" }) {
                    Write-Log "    SKIPPED: $($_.FullName) in protected game/update directory"
                } else {
